...
describe('.def_lazy', function() {
	...
	it('delays evaluation on function applications', function() {
		app.def_lazy('immediate', function(x) { this.v(x) })
...
describe('.def', function() {
	...
	xit('allows for vs-style function definition', function() {
		assert.equal(app.def('myadd x y', 'add x y').v('myadd 1 2'), 3)
	})
})
...
<div id='ys'><!--= repeat ys --><!--= put each --></div>\
	...
	it('can do nested loops', function() {
		var el = domify("<div><div><!--= repeat xs --><span><!--= repeat ys --><!--= put content --><span></div></div>")
		app.render(el)
...
describe('if', function() {
	...
	it('evals then_expr with predicate true', function() {
		assert.equal(app.v("if true 'yup' 'nope'"), 'yup')
	})

	it('evals else_expr with predicate false', function() {
		assert.equal(app.v("if false 'nope' 'yup'"), 'yup')
	})
})
...
describe("any", function() {
	it('returns the first truthy elem', function() {
		assert.equal(app.v("any false null undefined 0 420 false"), 420)
	})

	it('returns the last elem if no elems are truthy', function() {
		assert.equal(app.v("any null undefined 0 false"), false)
	})
})
...
describe('form_object', function() {
	...
	it('converts a form into an object', function() {
		...
		app.render(el)
		assert.deepEqual(app.formyay, {'hey[hey]': 'there', yo: ['first', 'second', 'third']})
	})

	xit('unflattens dotted names', function() {
		var el = domify("<div><form>\
			<!--= set 'form_dotted' form_object -->\
...
describe('.scope', function() {
	...
	it('does not change its parents data objects', function() {
		...
		app.def('x', {y: 99})
		child.def('x', {y: 33})
		assert.equal(app.v('x.y'), 99)
	})

	it('does not change its siblings data', function() {
		var sib1 = app.scope(), sib2 = app.scope()
		app.def('x', {y: 99})
		sib1.def('x', {y: 33})
		sib2.def('x', {y: 44})
		assert.equal(sib1.v('x.y'), 33)
		assert.equal(sib2.v('x.y'), 44)
		assert.equal(app.v('x.y'), 99)
	})
	...
	it('will render into the given scope name', function() {
		app._scopes = {}
		var el = domify("<div><!--= scope 'scope_x' --><!--= put hiyo --></div><div><!--= put hiyo --></div>")
		app.render(el)
		var scope_x = app.scope('scope_x')
		app.def('hiyo', 'from app')
		scope_x.def('hiyo', 'from scope_x ... ')
	...
	it("two scopes with the same keys will not render into each other's scopes", function() {
		app._scopes = {}
		var el = domify("<div><!--= scope 'scope_x' --><!--= put hiyo --></div>\
<div><!--= scope 'scope_y' --><!--= put hiyo --></div>")
		app.render(el)

		var scope_x = app.scope('scope_x'),
			scope_y = app.scope('scope_y')

		app.def('hiyo', 'from app')
		scope_x.def('hiyo', 'from scope_x ... ')
		scope_y.def('hiyo', 'from scope_y')
	...
	it("will render data properly when scoped in multiple elements", function() {
		app._scopes = {}
		var el = domify("<div><!--= scope 'scope_x' --><!--= put hiyo --></div>\
<div><!--= scope 'scope_x' --><!--= put hiyo --></div>")
		app.render(el)
	...
	it("will evaluate functions defined in the parent within the scope", function() {
		var el = domify("<div><!--= scope 'scope_x' --><span><!--= scope 'scope_y' --><!--= put scope_y_key --></span></div>")
		app.render(el)
		app.def('x_setter', 'in_app')
		var scope_x = app.scope('scope_x')
		var scope_y = scope_x.scope('scope_y')
		scope_y.def('scope_y_key', 'in_scope_y')
		assert.equal(el.textContent, 'in_scope_y')
	})
